  ...
  var server = http.createServer(function (req, res) {
     ...
     //register frames
       ...
       proxyRequest.addListener("response", function (response) {
         ...
         if (response.headers['content-type'] &&response.headers['content-type'].indexOf("text/html") != -1 && response.headers['content-length']){
           response.headers['content-length'] = parseInt(response.headers['content-length']) + includes.length
         }
