  ...
  var server = http.createServer(function (req, res) {
     ...
     if (pathname.indexOf('_jellyfish/poll') != -1) {
       if (_this.queue.length != 0) {
         ...
         if ((j.frame) && (j.frame == title)) {
           ...
           _this.log('output', 'dispatcher', job.meth+" to: "+_this.tid+" title: "+title);
           _this.log('command', job.meth, JSON.stringify(job));
       ...
       req.addListener("data", function (chunk) {
         ...
         _this.log('output', 'frame dying', JSON.stringify(msg))
       ...
       req.addListener("data", function (chunk) {
         ...
         _this.log('output', 'recording result', JSON.stringify(job));
         _this.log('report', job);
     ...
     //register frames
       ...
       req.addListener("data", function (chunk) {
         var data = chunk.toString();
         var msg = JSON.parse(data);
         _this.log('output', 'register frame', "tid:"+ _this.tid + " title: "+ msg.title);
       ...
       var clientError = function (e) { 
         if (!res._header) {
           res.writeHead(502, {})
           res.end();
           console.error("client error "+e.stack)  // This should be debug only.
         } else {
           res.end();
           console.error("client error after writeHead was called. "+e.stack) // This should not be debug, it should always display because it's really bad!
         }
       }  
    ...
    _this.log('output', 'started server', _this.tid+" on port "+_this.port);
    return server;
};
