...
firefox = function() {
  ...
  this.start = function(cb, url, port) {
    ...
    if (process.platform == "linux") {
      ff = path.normalize("/usr/bin/firefox");
    }
...
chrome = function() {
  ...
  this.start = function(cb, url, port) {
      ...
      if (process.platform == "linux") {
        chrome = path.normalize('/usr/bin/google-chrome');
      }
      if (port) {
        var proxy = "--proxy-server=127.0.0.1:"+port;
        var p = spawn(chrome, [proxy, "--incognito", "--temp-profile", "--no-first-run", "--disable-popup-blocking", "--homepage="+url]);        
      }
      else {
        var p = spawn(chrome, ["--incognito", "--temp-profile", "--no-first-run", "--disable-popup-blocking", "--homepage="+url]);        
      }
      _this.instance = p;
      if (cb) {
        cb(_this.instance);
