...
firefox = function() {
  ...
  this.start = function(cb, url, port) {
    ...
    var writePrefs = function(port, url, bid) {
    ...
    path.exists(ff, function(exists) {  
      if (!exists) {
        _this.version = 3;
        ff = path.normalize("/Applications/Firefox.app/Contents/MacOS/firefox-bin");
        var createProfile = spawn("cp", ["-R", "/Applications/Firefox.app/Contents/MacOS/defaults/profile/", "/tmp/browsers/"+bid])
         createProfile.on("exit", function(code) {
          writePrefs(port, url, bid);
...
chrome = function() {
  ...
  this.start = function(cb, url, port) {
      ...
      if (port) {
        ...
        var p = spawn(chrome, [proxy, "--incognito", "--disable-popup-blocking", "--homepage="+url]);        
      }
      else {
        var p = spawn(chrome, ["--incognito", "--disable-popup-blocking", "--homepage="+url]);        
      }
      _this.instance = p;
      if (cb) {
        cb(_this.instance);
